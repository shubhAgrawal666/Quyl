    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {courses.slice(0, 4).map((c) => (
              <div key={c._id} className="flex gap-4 items-start">
                ...
                <div className="text-right text-sm text-gray-500">
                  <div>{c.studentsEnrolled?.length || 0} enrolled</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <aside className="bg-white rounded-2xl p-6 shadow-sm border">
          <h3 className="text-md font-semibold mb-3">User distribution</h3>

          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium">Admins</span>
              <span className="text-lg font-bold text-orange-600">{totalAdmins}</span>
            </div>
